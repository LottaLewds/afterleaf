...
export const cdpEndpoint = () =>
  process.env.AFTERLEAF_CDP_ENDPOINT ?? DEFAULT_CDP_ENDPOINT;

export const afterleafGameUrl = () => {
  const url = process.env.AFTERLEAF_GAME_URL;
  if (!url)
    throw new Error(
      "AFTERLEAF_GAME_URL is unavailable; use the WSL wrapper or set it explicitly",
    );
  return url;
};
...
export const findAfterleafTarget = async () => {
  ...
  const targetSubstring =
    process.env.AFTERLEAF_CDP_TARGET ?? new URL(afterleafGameUrl()).host;
  const target = targets.find(
    (candidate) =>
      candidate.type === "page" && candidate.url.includes(targetSubstring),
  );
  if (!target)
    throw new Error(
      `No page target URL contains ${JSON.stringify(targetSubstring)}`,
    );
  return target;
};
