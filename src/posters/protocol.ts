...
export type PosterMediaRequest = {kind: "invalid"} | {id: string; kind: "media"} | {kind: "unscoped"};
...
export const posterMediaUrl = (id: string) =>
  `${POSTER_MEDIA_ENDPOINT_PREFIX}${Array.from(new TextEncoder().encode(id), (byte) =>
    byte.toString(16).padStart(2, "0"),
  ).join("")}.webp`;

export const parsePosterMediaRequest = (requestUrl: string): PosterMediaRequest => {
  let pathname: string;
  try {
  ...
  if (!pathname.startsWith(POSTER_MEDIA_ENDPOINT_PREFIX)) return {kind: "unscoped"};
...
export const parsePosterCatalog = (value: unknown): PosterCatalog => {
  if (!isRecord(value) || !Array.isArray(value.posters)) throw new Error("Poster catalog must contain a posters array");
...
export const parsePosterImportResponse = (value: unknown): PosterImportResponse => {
