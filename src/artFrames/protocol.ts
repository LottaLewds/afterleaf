...
export type ArtFrameMediaRequest = {kind: "invalid"} | {id: string; kind: "media"} | {kind: "unscoped"};
...
const isSafeId = (value: string) => {
  ...
  return (
    ...
    value.length <= 512 &&
    !value.includes("\\") &&
    segments.every((segment) => segment.length > 0 && segment !== "." && segment !== ".." && !segment.startsWith("."))
  );
};

export const artFrameMediaUrl = (id: string) =>
  `${ART_FRAME_MEDIA_ENDPOINT_PREFIX}${Array.from(new TextEncoder().encode(id), (byte) =>
    byte.toString(16).padStart(2, "0"),
  ).join("")}.webp`;

export const parseArtFrameMediaRequest = (requestUrl: string): ArtFrameMediaRequest => {
  let pathname: string;
  try {
  ...
  if (!pathname.startsWith(ART_FRAME_MEDIA_ENDPOINT_PREFIX)) return {kind: "unscoped"};
  ...
  if (encodedId.length === 0 || encodedId.length % 2 !== 0 || !/^[0-9a-f]+$/u.test(encodedId)) return {kind: "invalid"};
  try {
    const bytes = Uint8Array.from(encodedId.match(/.{2}/gu) ?? [], (byte) => Number.parseInt(byte, 16));
...
export const parseArtFrameCatalog = (value: unknown): ArtFrameCatalog => {
  ...
  const channels = value.channels.map((channel, channelIndex) => {
    ...
    if (channelIds.has(channel.id)) throw new Error("Art frame catalog has duplicate channel IDs");
    ...
    const images = channel.images.map((image, imageIndex) => {
      if (
        ...
        throw new Error(`Art frame image ${channelIndex}:${imageIndex} is invalid`);
      if (imageIds.has(image.id)) throw new Error("Art frame catalog has duplicate image IDs");
...
export const parseArtFrameImportResponse = (value: unknown, channelId: string): ArtFrameImportResponse => {
...
export const isSafeArtFrameChannelId = (value: string) => /^[a-z0-9](?:[a-z0-9-]{0,62}[a-z0-9])?$/u.test(value);
