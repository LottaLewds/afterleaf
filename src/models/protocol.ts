...
export type ModelMediaRequest = {kind: "invalid"} | {id: string; kind: "media"} | {kind: "unscoped"};
...
export const parseModelMediaRequest = (requestUrl: string): ModelMediaRequest => {
  let pathname: string;
  try {
  ...
  if (!pathname.startsWith(MODEL_MEDIA_ENDPOINT_PREFIX)) return {kind: "unscoped"};
...
export const parseModelCatalog = (value: unknown): ModelCatalog => {
  if (!isRecord(value) || !Array.isArray(value.models)) throw new Error("Model catalog must contain a models array");
