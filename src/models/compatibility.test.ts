...
import {modelNeedsSpecGlossConversion, prepareModelForThree} from "~/models/compatibility";
...
const createLegacySpecGlossGlb = async () => {
  ...
  document.createMaterial("legacy-material").setExtension("KHR_materials_pbrSpecularGlossiness", specGloss);
...
describe("model compatibility", () => {
  test("detects and converts legacy specular-glossiness materials", async () => {
    ...
    const prepared = await prepareModelForThree(sourcePath, join(directory, ".cache"));
