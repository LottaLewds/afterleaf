...
import {MAX_WORLD_SAVE_BODY_BYTES, WORLD_SAVE_ENDPOINT, WORLD_SAVE_SERVER_INSTANCE_HEADER} from "~/game/worldSaveHttp";
...
export const loadServerWorldSave = async (
  ...
  const serverInstanceId = response.headers.get(WORLD_SAVE_SERVER_INSTANCE_HEADER)?.trim();
  ...
  if (!response.ok) throw new Error(`World save download failed (${response.status})`);
...
export const saveServerWorldSave = async (
  ...
  if (bodyByteLength > MAX_WORLD_SAVE_BODY_BYTES) throw new Error("World save is too large to upload");
  const response = await fetcher(WORLD_SAVE_ENDPOINT, {
    body,
    cache: "no-store",
    credentials: "same-origin",
...
export const queueServerWorldSave = (save: WorldSaveV1, serverInstanceId: string, revision: string) => {
  const queuedSave = saveQueue.then(() => saveServerWorldSave(save, serverInstanceId, revision));
