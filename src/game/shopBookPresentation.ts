...
export class ShopBookPresentation {
  ...
  syncCarriedBookPresentation() {
    ...
    for (const [index, publicationId] of host
  ...
  writeHeldBookLocalPosition(index: number, output: Vector3) {
    ...
    for (let stackIndex = 1; stackIndex <= index; stackIndex += 1) {
      const previous = host
        .booksById()
        .get(host.carriedPublicationIds()[stackIndex - 1] ?? "");
      const current = host
        .booksById()
        .get(host.carriedPublicationIds()[stackIndex] ?? "");
  ...
  writeHeldBookLocalRotation(index: number, output: Quaternion) {
    this.#heldBookFanRotation.setFromAxisAngle(
      ...
      index * HELD_BOOK_FAN_ANGLE,
    );
    output
      .copy(this.#host.heldLocalRotation())
      .multiply(this.#heldBookFanRotation);
  }
  ...
  #animateInspectedBook(record: BookRecord, deltaSeconds: number) {
    ...
    host
      .inspection()
      .animateInspectionOpening(record, deltaSeconds, coverAnimationSpeed);
    if (
  ...
  #animateShelfPreview(
    ...
    const shelf =
      record.state.status === "shelved"
        ? host.spineShelfDefinitions().get(record.state.shelfId)
        : undefined;
  ...
  #respawnEscapedBook(publicationId: string, record: BookRecord) {
    ...
    host
      ...
      .respawnBook(
        ...
        host
          .bookLifecycle()
          .setPhysicsPose(record.basePosition, record.baseRotation),
      );
    host.markWorldStateDirty();
  }
}
