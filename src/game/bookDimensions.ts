...
export const physicalBookWidth = (aspectRatio: number | undefined, renderedHeight: number) => {
  const ratio = aspectRatio !== undefined && Number.isFinite(aspectRatio) ? aspectRatio : DEFAULT_BOOK_ASPECT_RATIO;
  return Math.min(MAX_BOOK_ASPECT_RATIO, Math.max(MIN_BOOK_ASPECT_RATIO, ratio)) * renderedHeight;
};

export const physicalBookDepth = (thicknessMillimeters: number | undefined, renderedHeight: number) => {
  const thickness =
