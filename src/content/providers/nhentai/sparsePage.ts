...
import {
  ...
  parseNhentaiSparseMetadata,
} from "./sparseMetadata";

type NhentaiSparsePageClient = Pick<
  NhentaiClient,
...
export const createNhentaiSparsePageMaterializer = (
  ...
  return async ({
    ...
    if (!remoteId || !/^\d+$/u.test(remoteId))
      throw new Error("nHentai publication has an invalid remote ID");
    if (
    ...
    if (pageNumber < 1 || pageNumber > pageCount)
      throw new Error("nHentai page is outside the publication");
    ...
    if (!galleryPromise) {
      galleryPromise = (async () => {
        ...
        try {
          const metadata = parseNhentaiSparseMetadata(
          ...
          await writeFile(
            metadataPath,
            `${JSON.stringify(
              createNhentaiSparseMetadata(gallery, metadataHash),
              null,
              2,
            )}\n`,
          );
