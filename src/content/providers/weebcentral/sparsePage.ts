...
type WeebCentralSparsePageClient = Pick<
  WeebCentralClient,
  "downloadPage" | "getPageList"
>;

export const createWeebCentralSparsePageMaterializer = (
  client: WeebCentralSparsePageClient,
) => {
  ...
  return async ({
    ...
    if (
      !publication.source?.metadataHash ||
      publication.source.metadataHash !== metadataHash
    )
      throw new Error("WeebCentral publication metadata is stale");
    if (pageNumber < 1 || pageNumber > pageCount)
      throw new Error("WeebCentral page is outside the publication");
    ...
    if (!pagesPromise) {
      pagesPromise = (async () => {
        try {
          const metadata = parseWeebCentralSparseMetadata(
          ...
          if (pageUrls.length !== pageCount)
            throw new Error(
              "Remote WeebCentral metadata changed; fetch the book again",
            );
          await writeFile(
            metadataPath,
            `${JSON.stringify(
              createWeebCentralSparseMetadata(remoteId, metadataHash, pageUrls),
              null,
              2,
            )}\n`,
          );
    ...
    if (!pageUrl)
      throw new Error("Remote WeebCentral page metadata is incomplete");
    try {
