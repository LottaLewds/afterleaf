...
type MangaDexSparsePageClient = Pick<
  MangaDexClient,
  "downloadPage" | "getAtHomeServer"
>;

export const createMangaDexSparsePageMaterializer = (
  client: MangaDexSparsePageClient,
) => {
  ...
  return async ({
    ...
    if (pageNumber < 1 || pageNumber > pageCount)
      throw new Error("MangaDex page is outside the publication");
    ...
    if (!serverPromise) {
      serverPromise = (async () => {
        try {
          const metadata = parseMangaDexSparseMetadata(
          ...
          if (server.chapter.data.length !== pageCount)
            throw new Error(
              "Remote MangaDex metadata changed; fetch the book again",
            );
          await writeFile(
            metadataPath,
            `${JSON.stringify(
              createMangaDexSparseMetadata(remoteId, metadataHash, server),
              null,
              2,
            )}\n`,
          );
    ...
    if (!filename)
      throw new Error("Remote MangaDex page metadata is incomplete");
    try {
