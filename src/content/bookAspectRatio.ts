...
export const bookAspectRatioSamplePageIndices = (
  pageCount: number,
  earlyInteriorCount = 2,
) => {
  ...
  return [
    ...new Set([
      ...Array.from(
        {length: Math.min(earlyInteriorCount, Math.max(0, lastPageIndex - 1))},
        (_, index) => index + 1,
      ),
      midpoint,
      midpoint + 1,
...
export const inferRepresentativeBookAspectRatio = (
  ...
  const representativeIndex = Math.min(
    ratios.length - 1,
    Math.floor(ratios.length * REPRESENTATIVE_RATIO_QUANTILE),
  );
  return boundedBookAspectRatio(
    ratios[representativeIndex] ?? fallbackAspectRatio,
  );
};
