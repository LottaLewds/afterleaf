...
const GARBAGE_DIRECTORIES = new Set(["asset-garbage", "snapshot-garbage", "source-garbage"]);

const garbageCollectorScript = fileURLToPath(new URL("../../../scripts/library-prune-snapshots.ts", import.meta.url));
...
export const pruneSnapshotGarbage = (garbageDirectory: string) => {
  ...
  if (!GARBAGE_DIRECTORIES.has(garbageKind))
    throw new Error("Snapshot garbage collection requires its dedicated directory");
  ...
  for (const entry of entries) {
    if (garbageKind !== "source-garbage" && (!entry.isDirectory() || !SNAPSHOT_ID_PATTERN.test(entry.name))) continue;
...
export const scheduleSnapshotGarbageCollection = (garbageDirectory: string) => {
  try {
    const child = spawn(process.execPath, [garbageCollectorScript, garbageDirectory], {
      detached: true,
      stdio: "ignore",
      windowsHide: true,
    });
