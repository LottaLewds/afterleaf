...
describe("blacklisted tag purge selection", () => {
  ...
  const publications = [officeLadyPublication, officePublication, yuriPublication, duplicateYuriPublication];

  test("matches normalized exact tags and deduplicates publications", () => {
    expect(findBlacklistedTagMatches(publications, ["  OFFICE   LADY ", "YURI"])).toEqual([
  ...
  test("does not treat a blacklisted tag as a substring", () => {
    expect(findBlacklistedTagMatches(publications, ["office"])).toEqual([officePublication]);
  });
