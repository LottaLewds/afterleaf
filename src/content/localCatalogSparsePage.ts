 ...
 */
export const localCatalogSourceRoots = async (
  ...
  return [
    ...
    ...new Set([
      ...
      ...(options.additionalCatalogDirectories ?? []).map((directory) =>
        resolve(directory),
      ),
    ]),
  ];
...
const resolvePublicationDirectory = (
...
export const materializeLocalCatalogReaderPage = async (
  ...
  if (!localSourceId)
    throw new Error("Publication is not backed by a local catalog folder");
  ...
  const document = parseLocalPublicationDocument(
    JSON.parse(
      await readFile(resolve(directory, LOCAL_PUBLICATION_MANIFEST), "utf8"),
    ) as unknown,
    LOCAL_PUBLICATION_MANIFEST,
  );
  if (document.id !== publication.id)
    throw new Error(
      "Publication manifest does not match the active catalog entry",
    );
  ...
  const derivative = await createReaderPageDerivative(
