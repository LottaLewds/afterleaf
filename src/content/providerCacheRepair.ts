...
const cachedProviderPublications = async (sourceDirectory: string, providerIds: ReadonlySet<string>) => {
  ...
  while (pendingDirectories.length > 0) {
    ...
    for (const entry of entries) {
      ...
      try {
        const document = parseLocalPublicationDocument(JSON.parse(await readFile(path, "utf8")) as unknown, path);
        const publicationDirectory = dirname(path);
        const providerId = document.source?.provider;
        if (document.id !== basename(publicationDirectory) || !providerId || !providerIds.has(providerId)) continue;
        publications.push({
          document,
          manifestPath: path,
          publicationDirectory,
          sourceId: portablePath(relative(sourceDirectory, publicationDirectory)),
        });
      } catch {
  ...
  return publications.toSorted((left, right) => left.sourceId.localeCompare(right.sourceId));
};

const assertAssetParentIsContained = async (publicationDirectory: string, assetPath: string) => {
  ...
  for (const segment of relativeParent.split(sep).filter(Boolean)) {
    ...
    try {
      if ((await lstat(current)).isSymbolicLink()) throw new Error(`Asset parent is a symbolic link: ${assetPath}`);
    } catch (error) {
...
const assetPageNumbers = (document: LocalPublicationDocument, pageCount: number) => {
  ...
  const add = (assetPath: string | undefined, pageNumber: number) => {
    ...
    if (existing !== undefined && existing !== pageNumber)
      throw new Error(`Asset ${JSON.stringify(assetPath)} represents multiple remote pages`);
    pageNumberByAsset.set(assetPath, pageNumber);
  };
  document.assets.pages.forEach((assetPath, index) => add(assetPath, index + 1));
  add(document.assets.front, 1);
  add(document.assets.back, pageCount);
  return pageNumberByAsset;
};

const repairPublication = async (candidate: CachedProviderPublication, providerRegistry: LibraryProviderRegistry) => {
  const providerId = candidate.document.source?.provider;
  const metadataHash = candidate.document.source?.metadataHash;
  if (!providerId || !metadataHash) throw new Error("the cached publication has incomplete provider metadata");
  const provider = await providerRegistry.load(providerId);
  const materializePage = provider.materializePage;
  if (!materializePage) throw new Error("the provider does not support cached-page repair");
  const pageCount = candidate.document.pageCount ?? candidate.document.assets.pages.length;
  const pageNumberByAsset = assetPageNumbers(candidate.document, pageCount);
  const pagePlan = createRepresentativePagePlan(pageCount, Math.max(1, candidate.document.assets.pages.length));
  ...
  const workers = await Promise.allSettled(
    Array.from({length: Math.min(3, requestedPageIndexes.length)}, async () => {
      while (nextIndex < requestedPageIndexes.length) {
        ...
        if (!(await readImageDimensions(bytes))) throw new Error(`page ${pageIndex + 1} is not a supported image`);
  ...
  const bytesByPageNumber = new Map(downloadedPages.map(({bytes, pageIndex}) => [pageIndex + 1, bytes]));
  ...
  try {
    for (const [assetPath, pageNumber] of pageNumberByAsset) {
      ...
      const targetPath = await assertAssetParentIsContained(candidate.publicationDirectory, assetPath);
      await mkdir(dirname(targetPath), {recursive: true});
      const stagedPath = `${targetPath}.repair-${randomUUID()}`;
      await writeFile(stagedPath, bytes);
      stagedAssets.push({stagedPath, targetPath});
    }
    for (const asset of stagedAssets) await rename(asset.stagedPath, asset.targetPath);
    const document = await finalizeProviderPublicationDocument(candidate.document, downloadedPages);
    ...
    await Promise.all(
      [...stagedAssets.map(({stagedPath}) => stagedPath), ...(stagedManifest ? [stagedManifest] : [])].map((path) =>
        rm(path, {force: true}),
      ),
    );
  }
...
export const repairCachedProviderPublications = async (
  ...
  const providerIds = new Set(options.providerRegistry.descriptors().map(({id}) => id));
  const publications = await cachedProviderPublications(options.sourceDirectory, providerIds);
  const diagnostics: ProviderCacheRepairDiagnostic[] = [];
  let repairedCount = 0;
  if (publications.length === 0) return {diagnostics, failedCount: 0, repairedCount, requestedCount: 0};
  options.onProgress?.(`Deep remote repair: 0/${publications.length} cached publications complete`);
  for (const [index, publication] of publications.entries()) {
    const providerId = publication.document.source?.provider;
    if (!providerId) continue;
    options.onProgress?.(`Deep remote repair: repairing ${publication.sourceId} (${index + 1}/${publications.length})`);
    try {
